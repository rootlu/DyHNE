...
% M_apa = (speye(size(W_apa,1)) - W_apa)' * (speye(size(W_apa,1)) - W_apa);
...
% M_apcpa = (speye(size(W_apcpa,1)) - W_apcpa)' * (speye(size(W_apcpa,1)) - W_apcpa);
% [apcpa_embedding, U_apcpa, Lambda_apcpa] = DHINOffline(L_apcpa+gamma*M_apcpa, D_apcpa,k);
% save ./data/dblp/result/apcpa_embedding.mat apcpa_embedding;

% daptpa = sum(W_aptpa,2);
% D_aptpa = diag(daptpa);
% L_aptpa = D_aptpa- W_aptpa;  
% W_aptpa = NormalizeAdj(W_aptpa,0,2);
% M_aptpa = (speye(size(W_aptpa,1)) - W_aptpa)' * (speye(size(W_aptpa,1)) - W_aptpa);
