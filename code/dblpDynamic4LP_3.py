...
class DataHelper:
    ...
    def split_data_three_parts(self,no_iso,split_ratio):
        if no_iso:
            print('split with no_iso...')
            pa_adj_mtx = self.pa_adj_mtx
            target_num = split_ratio * len(self.pa_lines)
        ...
        print('#train pa: {}, #test pa: {}'.
              format(len(self.train_pa_lines), len(self.test_pa_lines)))
...
if __name__ == '__main__':
    dh = DataHelper(data_dir='../data/dblp/oriData/',
                    baseline_data_dir='../baseline/dblpDynamic_lp_no_iso/',
                    our_data_dir='../data/dblpDynamic_lp_no_iso')
    dh.load_data()

    dh.split_data_three_parts(no_iso=True)
