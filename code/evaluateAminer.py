...
class Evaluation:
    ...
    def kmeans_nmi(self,cluster_k):
        x = []
        y = []
        with open('../data/aminer/oriData/author_label.txt', 'r') as author_name_label_file:
    ...
    def link_prediction_with_hit(self,hist_k):
        ...
        with open('../data/aminer_lp/test_aa_pos_neg.txt','r') as ld_f:
    ...
    def get_good_author(self):
        self.good_author_list = []
        with open('../data/aminer_lp_no_iso/good_author_4_test.txt') as ga_file:
            for line in ga_file:
                self.good_author_list.append(line.strip())

    def pre_4_link_prediction(self,data_type):
        # print('prepare {} data for link prediction...'.format(data_type))
        x = []
        y = []
        with open('../data/aminerDynamic_lp/'+data_type+'_aa_pos.txt', 'r') as aa_pos_f:
        # with open('../data/aminerDynamic_lp/'+data_type+'_aa_pos.txt', 'r') as aa_pos_f:
            for line in aa_pos_f:
                tokens = line.strip().split('\t')
                if self.name_emb_dict.has_key(int(tokens[0])) and self.name_emb_dict.has_key(int(tokens[1])):
                # if tokens[0] in self.good_author_list and tokens[1] in self.good_author_list:
        ...
        with open('../data/aminerDynamic_lp/'+data_type+'_aa_neg.txt', 'r') as aa_neg_f:
        # with open('../data/aminerDynamic_lp/'+data_type+'_aa_neg.txt', 'r') as aa_neg_f:
        ...
        # return x[0:2500]+x[-2500:],y[0:2500]+y[-2500:]
        return x,y
    ...
    def pre_4_node_recommendation(self,author_embeddings,conf_embeddings,author_start_end_id,conf_start_end_id):
        ...
        key = filter(lambda k: k.startswith('_') is False, a_embeddings_mat.keys())[0]
    ...
    def node_recommendation(self,hit_k):
        with open('../data/aminer_nr/test_ac_pos_neg.txt', 'r') as test_ac_p_n_f:
    ...
    def get_static_author(self):
        self.static_authors = []
        with open('../baseline/aminerDynamic/dw.node2id_2000','r') as d_f:
            for line in d_f:
                tokens = line.strip().split('\t')[0]
                if tokens[0] == 'a':
                    self.static_authors.append(tokens[1:])

    def new_cl(self):
        x_train, x_valid, y_train, y_valid = [],[],[],[]
        new_nodes = []
        with open('../aminer_delta_nodes.txt', 'r') as new_node_file:
...
if __name__ == '__main__':
    ...
    # train_ratio = [0.8]
    # embeddings_data = '../data/aminer/result/unify_0.25apa+0.5apcpa+0.25aptpa_embedding_d200.mat'
    ...
    # print('===== link prediction =====')
    # embeddings_data = '../data/aminer_lp_no_iso/result/unify_0.25apa+0.5apcpa+0.25aptpa_embedding_lp.mat'
    # print(embeddings_data)
    # exp = Evaluation(embeddings_data)
    # exp.get_good_author()
    # exp.load_embeddings()
    ...
    # embeddings_data = '../data/aminerDynamic/result/2004_0.25apa+0.5apcpa+0.25aptpa_embedding.mat'
    # print(embeddings_data)
    # exp = Evaluation(embeddings_data)
    # exp.load_embeddings()
    # for t_r in train_ratio:
    ...
    # for t_r in train_ratio:
    #     for i in xrange(10):
    #         embeddings_data = '../data/aminerDynamic/result/2005_' + str(i+1) + '_0.25apa+0.5apcpa+0.25aptpa_embedding.mat'
    #         print(embeddings_data)
    #         exp = Evaluation(embeddings_data)
    #         exp.load_embeddings()
    ...
    # # TODO: 更改link_prediction_with_auc 中正负样本文件名
    # embeddings_data = '../data/aminerDynamic_lp/result/full_apa+apcpa+aptpa_embedding_lp.mat'
    # print(embeddings_data)
    # exp = Evaluation(embeddings_data)
    # # exp.get_good_author()
    # exp.load_embeddings()
    ...
    #
    # for i in xrange(10):
    #     embeddings_data = '../data/aminerDynamic_lp/result/2004_' + str(i+1) + '_apa+apcpa+aptpa_embedding_lp.mat'
    #     print(embeddings_data)
    #     exp = Evaluation(embeddings_data)
    #     exp.load_embeddings()
    #     exp.link_prediction_with_auc()
    #
    # embeddings_data = '../data/aminerDynamic_lp/result/retrain_apa+apcpa+aptpa_embedding.mat'
    # print(embeddings_data)
    # exp = Evaluation(embeddings_data)
    # # exp.get_good_author()
    # exp.load_embeddings()
    ...
    print ('AMiner')
    print ('retrain...')
    embeddings_data = '../data/aminer/result/unify_apa+apcpa+aptpa_embedding.mat'
    exp = Evaluation(embeddings_data)
    exp.load_embeddings()
    exp.new_cl()
    ...
    # dynamic
    print ('dynamic...')
    # embeddings_data = '../data/aminer/result/unify_apcpa+0.5apa_embedding_g0.1.mat'
    # exp = Evaluation(embeddings_data)
    # exp.load_embeddings()
    # exp.new_cl()
    print ('Macro_F1_score:{}'.format(0.903466239273))
    print ('Micro_F1_score:{}'.format(0.902034134512))
