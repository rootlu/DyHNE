...
class DBLP4LinkPrediction:
    ...
    def load_data(self):
        ...
        print ('#paper:{}, #author:{}, #conf:{}, #term:{}, #year:{}'.
               format(len(self.paper_list), len(self.author_list),
                      len(self.conf_list), len(self.term_list),len(self.year_list)))
    ...
    def process_data(self):
        ...
        """
        print('process data...')
        for p,y in self.paper_year.items():
            if len(y) > 1:
                print('error paper {} with more than 2 years',format(p))
            else:
                pass
