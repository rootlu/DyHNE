...
class DataHelper:
    ...
    def process_data(self,current_review_list,time_step):
        ...
        with open(os.path.join('../data/yelpWWWDynamic/oriData', 'user_review'+str(time_step)+'.txt'),'w') as pa_file:
            for line in self.ur_lines:
                token = line.strip('\n').split('\t')
                user_name = 'u' + token[0]
                review_name = 'r' + token[1]
        ...
        with open(os.path.join('../data/yelpWWWDynamic/oriData', 'business_review'+str(time_step)+'.txt'),'w') as pc_file:
            for line in self.br_lines:
                token = line.strip('\n').split('\t')
                business_name = 'b' + token[0]
                review_name = 'r' + token[1]
    ...
    def get_data_4_dane(self,current_data_dict,time_step):
        ...
        scipy.io.savemat(os.path.join(self.baseline_fold, 'dane_csr_' + str(time_step) + '.mat'),
...
if __name__ == '__main__':
    dh = DataHelper(data_dir='../data/yelpWWW/oriData/',
                    baseline_dir='../baseline/yelpWWWDynamic/',
                    our_dir='../data/yelpWWWDynamic/')
    dh.load_data()
